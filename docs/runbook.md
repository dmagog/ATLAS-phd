...
## 8.5. Rate-limit recovery (login / LLM)

### Login: «студент пишет — не могу войти, выдаёт 429»

`/auth/login` блокирует:
- 5 неудачных попыток на пару (IP, email) за 5 мин;
- 50 неудачных попыток с одного IP (по любым email) за 5 мин.

Это защита от brute-force. Большинство случаев решается ожиданием
5 минут — счётчик сбросится автоматически. Успешный логин сбрасывает
(IP, email)-счётчик мгновенно.

**Когда нужно вмешаться руками** — почти никогда: лимиты живут
в памяти процесса (`src/atlas/core/rate_limit.py:_per_ip_email`,
`_per_ip`). Перезапуск контейнера обнуляет всё:

```bash
docker compose restart app
# downtime ~10-15s, все login-счётчики сбрасываются
```

Если лимит срабатывает массово (несколько пользователей одновременно
жалуются на 429) — проверь логи на `login_failed` и убедись, что это
не реальная атака:

```bash
docker compose logs app --since 30m | grep login_failed
# нормально: единичные с разных IP
# подозрительно: десятки с одного IP / на один email
```

### LLM-квота: «вылезает 429 на /qa или /chat»

Per-user квота 60 запросов/час. Решение — ждать; super-admin
исключён из квоты, поэтому если нужно срочно протестировать
поведение системы под валидным аккаунтом, используй super-admin
JWT (см. §8 invite-flow → роль).

При горизонтальном scale-out (>1 worker) счётчик per-process —
лимит может срабатывать неожиданно поздно или рано. Это известное
ограничение, миграция на Redis-based limiter — в roadmap.

См. [docs/security.md §2](security.md) для полного описания лимитов.
