...
**Сценарий 4в — типичный «Хорошо» (4.0/5) с подробной рекомендацией:** открыть попытку ivanov на «Принципы Ферма и Гюйгенса» — score 4.0 (raw 3.96), verdict «Хорошо». Точность 4.4, полнота 3.5 (тянет вниз), терминология 4.5. Резюме на 3 предложения с конкретной рекомендацией «изучить применения принципа Ферма и его следствия».

![selfcheck-good](screenshots/after/05c-selfcheck-good.png)

**Тезис «средний случай»:** рубрика не бинарна — показывает где сильно (точность/терминология 4.4-4.5), где слабо (полнота 3.5). Резюме даёт **адресную рекомендацию** что доучить, а не общее «учите ещё».

**Сценарий 4г — не зачёт (1.0/5):** открыть попытку `raskolnikov@optics.demo` на «Поляризация света» — verdict «Нужно повторить», все критерии в красном, error-теги «брюстер», «двулучепреломление».
