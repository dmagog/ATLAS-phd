...
Папки внутри `design/`:
- [`design/screenshots/after/`](design/screenshots/after/) — 15 PNG актуальных скриншотов (login → chat → source-modal → refusal → selfcheck старт/Отлично/Хорошо/Не-зачёт → supervisor → tenant-admin → eval → tenants → invites → semicon proof)
